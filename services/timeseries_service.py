...
class TimeSeriesEngine:
    ...
    def __init__(
        ...
        print("[SpiderCrypt] TimeSeriesEngine initialized", flush=True)
        print(
            f"   sensitivity={sensitivity}x baseline_window={baseline_window} points",
            flush=True,
        )
    ...
    def analyze(
        ...
        print(f"  [stats] {len(streams)} signals | {total_points} points | {len(all_alerts)} raw alerts")
        ...
        print(f"\n[done] analysis finished in {duration}s")
    ...
    def _adapt_time_window(self, entity_id: str, since_ms: int, window_hours: int) -> int:
        ...
        if lag_days > 7:
            print(f"  [info] data lag ~{lag_days:.0f}d -- window anchored on last point")
