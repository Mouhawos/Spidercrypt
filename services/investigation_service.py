...
class InvestigationEngine:
    ...
    def __init__(self, signing_key_b64: str | None = None):
        ...
        if signing_key_b64 and HAS_NACL:
            ...
            print("[warn] PyNaCl not installed -- Ed25519 signatures disabled")

    # ── Chargement ────────────────────────────────────────────────────────────

    def load_audit_events(self, path: str | pd.DataFrame) -> "InvestigationEngine":
        self._df_audit = self._load(path)
        print(f"  [audit] loaded {len(self._df_audit):,} events")
        return self

    def load_transactions(self, path: str | pd.DataFrame) -> "InvestigationEngine":
        self._df_tx = self._load(path)
        print(f"  [tx] loaded {len(self._df_tx):,} transactions")
        return self

    def load_audit_events_df(self, df: pd.DataFrame) -> "InvestigationEngine":
        return self.load_audit_events(df)

    def load_transactions_df(self, df: pd.DataFrame) -> "InvestigationEngine":
        return self.load_transactions(df)
    ...
    def _compute_time_window(self, days_back: int) -> tuple[int, int]:
        ...
        if now_ms - data_max_ms < seven_days_ms:
            ...
            print(f"  [info] data lag ~{lag_days}d -- window anchored on last timestamp")
    ...
    def investigate(
        ...
        print(f"  [anomalies] {len(anomalies)} signals")
        ...
        print(f"\n[done] investigation finished in {duration}s")
    ...
    def _diagnose_empty_timeline(
        ...
        print("\n  [warn] empty timeline -- diagnostic:")
        ...
        if self._df_audit is not None and "timestamp_ms" in self._df_audit.columns:
            ...
            print(f"     window_ms    : [{since_ms} -> {until_ms}]")
            ...
            if actor_id and "acteur_id" in self._df_audit.columns:
                ...
                print(f"     events for actor {actor_id!r} (all dates) : {len(actor_events)}")
                if len(actor_events) == 0:
                    sample = self._df_audit["acteur_id"].unique()[:5].tolist()
                    print(f"     -> actor_id not found. sample ids : {sample}")
